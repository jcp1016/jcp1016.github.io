...
<br /><br />
Source:
[Wired magazine](http://www.wired.com/2010/11/ff_311_new_york/)
...
<br />
Make a map showing poverty levels by neighborhood.<br />
...
```
    by_dc <- group_by( data, CD_Name, ComplaintType )<br />
    s1 <- summarize( by_dc, n=sum(n) )<br />
    t1 <- melt(s1)<br />
    t2 <- cast(t1, ComplaintType ~ CD_Name, sum, margins=TRUE)<br />
    names(t2)[ncol(t2)] <- "Total"<br />
    t3 <- filter(t2, Total > 5000)<br />
    t4 <- t3[order(desc(t3$Total)),]<br />
    t4 <- t4[-1,]<br />
    row.names(t4) <- t4$ComplaintType<br />
    t4 <- t4[,-1]<br />
    t4 <- t4[,-ncol(t4)]<br />
    data_matrix <- data.matrix(scale(t4))<br />
    heatmap.2(data_matrix,
          Rowv=NA, Colv=NA,
          dendrogram='none',
          scale='none',
